...
app.use('/slides', express.static(__dirname + '/views/slides'));
...
app.get('/teacher/', (request, response) => isTeacher(request, settings => {
	// Retrieve statistical data

		response.render('teacher/dashboard.html', { selected: 'dashboard', });
}, () => {
	// Redirect to login page
	response.redirect('/');
}));
app.get('/teacher/creCourse', (request, response) => isTeacher(request, settings => {
	// Retrieve statistical data

			response.render('teacher/creCourse.html', { selected: 'creCourse', });
}, () => {
	// Redirect to login page
	response.redirect('/');
}));
...
//Post data course
app.post('/teacher/creCourse', (request, response) => isTeacher(request, settings => {
	const { courseName, courseDescription, coursePrice, courseLevel, courseDuration } = request.body;
  
	const sql = `INSERT INTO courses (course_name, course_description, course_price, course_level, course_duration, account_id) VALUES (?, ?, ?, ?, ?, ?)`;
	const values = [courseName, courseDescription, coursePrice, courseLevel, courseDuration, request.session.account_id];
  
	connection.query(sql, values, (err, result) => {
	  if (err) {
		console.error(err);
		response.status(500).send('An error occurred while adding the course to the database.');
		return;
	  }
	  const message = `Course "${courseName}" has been added successfully.`;
	  const redirectUrl = '/teacher';
  ...
  app.get('/teacher/creLesson', (request, response) => isTeacher(request, settings => {
	// Fetch the list of courses belonging to the user
	const userId = request.session.account_id; // replace with your user ID retrieval logic
	connection.query(
	  'SELECT course_id, course_name FROM courses WHERE account_id = ?',
	  [userId],
	  (err, results) => {
		if (err) throw err;
  
	// Render the form with the list of courses
		response.render('teacher/creLesson.html', { selected: 'creCourse', courses: results});
	  }
	);
}, () => {
	// Redirect to login page
  ...
  app.post('/teacher/creLesson', (request, response) => isTeacher(request, settings => {
	// Extract the lesson data from the request body
	const { lessonName, lessonDescription, lessonContent, courseId } = request.body;
  
	// Insert the new lesson into the database
	connection.query(
	  'INSERT INTO lessons (lesson_name, lesson_description, lesson_content, course_id) VALUES (?, ?, ?, ?)',
	  [lessonName, lessonDescription,lessonContent, courseId],
	  (err, result) => {
		if (err) throw err;
  ...
  app.get('/teacher/creQuiz', (request, response) => isTeacher(request, settings => {
	// Fetch the list of courses belonging to the user
	const userId = request.session.account_id; // replace with your user ID retrieval logic
	connection.query(
	  'SELECT course_id, course_name FROM courses WHERE account_id = ?',
	...
	// Render the form with the list of courses
		response.render('teacher/creQuiz.html', { selected: 'creQuiz', courses: results});
	  }
	);
}, () => {
	// Redirect to login page
...
app.post('/teacher/creQuiz', upload.none(), (req, res) => {
	...
	for (let i = 1; req.body[`question${i}`]; i++) {
	  quizzes.push({
		...
		rightAnswer: req.body[`rightAnswer${i}`]
	  });
	}
	...
	if (!quizzes.length) {
	  res.status(400).send('No quizzes to add');
	  return;
	}
  
	let query = 'INSERT INTO quiz (course_id, quiz_question, quiz_as1, quiz_as2, quiz_as3, quiz_as4, quiz_ras) VALUES ?';
	let values = quizzes.map(quiz => [
	  quiz.courseId,
	  quiz.question,
	  ...quiz.options,
	  quiz.rightAnswer
	...
	connection.query(query, [values], function (err, result) {
	  if (err) {
		...
		res.status(200).send('Quizzes successfully added');
	  }
	});
  });
...
const isTeacher = (request, callback, callback2) => {
	isLoggedin(request, () => {
		if (request.session.account_role == 'Teacher') {
			callback();
		} else {
			callback2();
		}
	...
	// Listen on port 3000 (http://localhost:80/)
	app.listen(3000);